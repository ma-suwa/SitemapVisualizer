    ...
    urls.forEach(url => {
        ...
        try {
            ...
            path.forEach((segment, index) => {
                ...
                // 【修正】後から来た短いURLによって下層ノードが消去されるバグを防ぐため、
                // index === path.length - 1 の時に children を空にする処理を削除しました。
...
const defaultCsvPath = 'default.csv'; // デフォルトのCSVファイルのパス
...
fetch(defaultCsvPath)
    ...
    .catch(error => console.error('デフォルトCSVの読み込みエラー:', error));
});
...
// Tweakpaneの設定
((folder) => {
  folder.addInput(PARAMS, 'Depth', { step: 1, min: 0, max: 15 });
